...
function q1() {
    ...
    const color = d3.scaleOrdinal()
        ...
        .range(["#00c2c7", "#ff6f61", "#555555", "#f5c542", "#3f3f3f"]);
    ...
    const margin = {top: 20, right: 200, bottom: 40, left: 320}; // giảm top margin
    const width = 750; // tăng thêm cho legend
    const height = 450;
    ...
    const legend = d3.select("#chart svg")
        .append("g")
        .attr("class", "legend")
        .attr("transform", `translate(${width + margin.left + 45}, ${margin.top})`);
...
function q12() {
    ...
    svg.append("g")
        .attr("transform", `translate(0,${height})`)
        .call(d3.axisBottom(x).tickFormat(""))  // ẩn nhãn
        .selectAll("text")
        .remove();  // loại bỏ text
